import{i as M}from"./vendor-db25513e.js";import{musclesGroup as C,bodyPart as H}from"./api-8d58433a.js";const N=document.querySelector(".exercise-cards-list"),S=document.querySelector(".exercise-cards-list"),h=document.querySelector(".exercise-pages-counter"),U=document.querySelector(".exercise-form"),w=document.querySelector(".exercise-input-button"),y=document.querySelector(".exercise-input"),A=document.querySelectorAll(".exercise-button"),o=document.querySelector(".exercise-parts-list"),b=document.querySelector(".exercise-log-span"),p=document.querySelector(".exercise-title-span");document.querySelector(".exercise-part-button");document.querySelector(".pop-add-fav");document.querySelector(".pop-ex-close-btn");document.querySelector(".pop-backdrop");document.querySelector(".backdrop");document.querySelector(".pop-rating-btn");document.querySelector(".pop-up-close-btn");document.querySelector(".pop-ex-info");document.querySelector(".modal123");document.querySelector(".pop-backdrop.is-open");let g,L="",m=[],f,k=0,q=0,I=0,E=0;localStorage.removeItem("searchInput");localStorage.removeItem("searchQuery");sessionStorage.removeItem("data");N.addEventListener("click",async e=>{e.preventDefault(),m=[],g=1;try{const t=(await C(e)).results;m=[...t],p.innerHTML="",p.textContent=`${t[0].target}`.charAt(0).toUpperCase()+`${t[0].target}`.slice(1),sessionStorage.setItem("data",JSON.stringify({results:t})),k=0;for(let s=0;s<8;s++)k++;if(t.length===0)throw new Error({title:"No Results",message:"No images found. Please try a different search term."});p.style.visibility="visible",S.style.display="none",o.style.display="flex",b.style.visibility="visible",o.innerHTML=t.reduce((s,r,c)=>s+`
            ...
            <div class="part-container">
                <div class="exercise-head-container">
                    ...
                    <svg class="exercise-rating-icon" width="18" height="18">
                        <use href="/energy_flow/assets/icons.svg#icon-star"></use>
                    </svg>
                    </span>
                ...
                <use href="/energy_flow/assets/icons.svg#icon-arrow-right"></use>
                </svg>
                </button>
                </a>
                </div>
                ...
                <svg class="exercise-part-icon" width="24" height="24">
                    <use href="/energy_flow/assets/icons.svg#icon-running-man"></use>
                </svg>
        ...
        `,"")}catch(t){console.error("Error:",t)}});A.forEach(e=>{e.addEventListener("click",i=>{i.preventDefault();const t=i.target.textContent;localStorage.removeItem("searchInput"),localStorage.removeItem("searchQuery"),t==="Muscles"?(y.style.display="none",w.style.display="none",o.style.display="none",S.style.display="flex",b.style.visibility="hidden",p.style.visibility="hidden"):t==="Body parts"?(b.style.visibility="hidden",o.innerHTML="",y.style.display="flex",w.style.display="flex",o.style.display="flex",S.style.display="none",h.style.display="none",p.style.visibility="hidden"):(y.style.display="none",w.style.display="none",o.style.display="none",b.style.visibility="hidden",p.style.visibility="hidden")})});U.addEventListener("submit",async function(e){e.preventDefault(),m=[],g=1;try{const i=y.value,s=(await H(i)).results;if(m=[...s],sessionStorage.setItem("data",JSON.stringify({results:s})),s.length===0){o.innerHTML="",console.log(s.length),o.insertAdjacentHTML("beforeend","<li class='no-result-list'><p class='no-result'>Unfortunately,<span class='no-result-span'> no results</span> were found.You may want to consider other search options to find the exercise you are looking for. Our range is wide and you have the opportunity to find more options that suit your needs.</p>"),h.style.display="none";return}else{p.innerHTML="",p.textContent=`${s[0].bodyPart}`.charAt(0).toUpperCase()+`${s[0].bodyPart}`.slice(1),p.style.visibility="visible",b.style.visibility="visible",h.style.display="flex",I=0;for(let r=0;r<8;r++)I++;o.innerHTML=s.reduce((r,c,d)=>r+`
        <li class="exercise-parts">
            <div class="part-container">
                <div class="exercise-head-container">
                    <span class="exercise-badge">WORKOUT</span>
    ...
    <div class="pop-ex-modal">
      <button class="pop-ex-close-btn">
        ...
        >
          <use href="/energy_flow/assets/icons.svg#icon-close"></use>
        </svg>
      </button>
        ...
        <div class="pop-btns-container">
          <button class="pop-add-fav">
            Add to favorites<svg class="heart-icon" width="18" height="18">
              <use href="/energy_flow/assets/icons.svg#icon-heart"></use>
            </svg>
          </button>
...
`}o.addEventListener("click",async e=>{e.preventDefault();const i=e.target.id,t=JSON.parse(sessionStorage.getItem("data"));if(e.target.tagName=="BUTTON"&&i==i){const s=t.results[i];P(s),J(s)}});function J(e){function i(l){console.log(l.target);const n=JSON.parse(localStorage.getItem("exercises"))||[];console.log(123),n.push(e),localStorage.setItem("exercises",JSON.stringify(n)),[...n];const a=document.querySelector(".pop-add-fav");a&&a.removeEventListener("click",i),document.querySelector(".pop-ex-close-btn"),M.success({title:"Excellent choice!",message:"The exercise has been successfully added to your favorites. Get ready for endless inspiration and great results!",position:"center"})}const t=document.querySelector(".pop-add-fav");t?t.addEventListener("click",i):console.error("Button not found");const s=document.querySelector(".pop-backdrop.is-open");async function r(){await P(e),v.innerHTML="",s.classList.remove("is-open")}s&&s.addEventListener("click",r),window.addEventListener("keydown",function(l){l.key==="Escape"&&(v.innerHTML="")});function c(){v.innerHTML=""}const d=document.querySelector(".pop-ex-close-btn");d?d.addEventListener("click",c):console.error("Close button not found")}const x=JSON.parse(localStorage.getItem("exercises"));console.log(x);const $=document.querySelector(".fav-list"),B=document.querySelector(".favorites");function T(e){x.length===0?R():($.innerHTML=x.slice(0,8).map(({bodyPart:i,name:t,target:s,burnedCalories:r},c)=>`
        ...
        <div class="exercise-head-container">
          ...
          <button class="exercise-trash-button">
            <svg class="exercise-trash-icon" width="16" height="16">
              <use href="/energy_flow/assets/icons.svg#icon-trash"></use>
            </svg>
          </button>
          <a
  ...
  `).join(""),O())}T();x.length>8&&D();B.addEventListener("click",function(e){if(e.target.classList.contains("exercise-number-button")){const i=parseInt(e.target.id),t=(i-1)*8,s=i*8;$.innerHTML=x.slice(t,s).map(({bodyPart:r,name:c,target:d,burnedCalories:l},n)=>`
    <li class="exercise-parts">
      <div class="part-container">
        <div class="exercise-head-container">
          <span class="exercise-badge">WORKOUT</span>
  ...
  `).join(""),O()}});function O(){document.querySelectorAll(".exercise-trash-button").forEach((i,t)=>{i.setAttribute("data-index",t),i.addEventListener("click",s=>{const r=s.currentTarget.getAttribute("data-index");x.splice(r,1),localStorage.setItem("exercises",JSON.stringify(x)),T()})})}T();function R(){$.innerHTML=`
...
//# sourceMappingURL=exersises-a79b1d6f.js.map
