...
<path fill="#f6f6f6" style="fill: var(--color1, #f6f6f6)" d="M20.919 10.126c1.166 0 2.111-0.924 2.111-2.063s-0.945-2.063-2.111-2.063c-1.166 0-2.111 0.924-2.111 2.063s0.945 2.063 2.111 2.063z"></path>
        ...
        `,"")}catch(c){console.error("Error:",c)}});E.forEach(e=>{e.addEventListener("click",t=>{t.preventDefault();const c=t.target.textContent;localStorage.removeItem("searchInput"),localStorage.removeItem("searchQuery"),A(t),c==="Muscles"?(x.style.display="none",S.style.display="none",l.style.display="none",$.style.display="flex",v.style.visibility="hidden",d.style.visibility="hidden",h.style.display="flex"):c==="Body parts"?(v.style.visibility="hidden",l.innerHTML="",x.style.display="flex",S.style.display="flex",l.style.display="flex",$.style.display="none",h.style.display="none",d.style.visibility="hidden"):(x.style.display="none",S.style.display="none",l.style.display="none",v.style.visibility="hidden",d.style.visibility="hidden")})});j.addEventListener("submit",async function(e){e.preventDefault(),y=[],f=1;try{const t=x.value,s=(await O(t)).results;if(y=[...s],sessionStorage.setItem("data",JSON.stringify({results:s})),s.length===0){l.innerHTML="",console.log(s.length),l.insertAdjacentHTML("beforeend","<li class='no-result-list'><p class='no-result'>Unfortunately,<span class='no-result-span'> no results</span> were found.You may want to consider other search options to find the exercise you are looking for. Our range is wide and you have the opportunity to find more options that suit your needs.</p>"),h.style.display="none";return}else{d.innerHTML="",d.textContent=`${s[0].bodyPart}`.charAt(0).toUpperCase()+`${s[0].bodyPart}`.slice(1),d.style.visibility="visible",v.style.visibility="visible",h.style.display="flex",T=0;for(let i=0;i<8;i++)T++;l.innerHTML=s.reduce((i,a,g)=>i+`
        <li class="exercise-parts">
            <div class="part-container">
                <div class="exercise-head-container">
                    <span class="exercise-badge">WORKOUT</span>
...
//# sourceMappingURL=exersises-2bd86761.js.map
