import{i as j}from"./vendor-55aaf39b.js";import{musclesGroup as N,bodyPart as U}from"./api-8d58433a.js";const A=document.querySelector(".exercise-cards-list"),M=document.querySelector(".exercise-cards-list"),f=document.querySelector(".exercise-pages-counter"),J=document.querySelector(".exercise-form"),k=document.querySelector(".exercise-input-button"),b=document.querySelector(".exercise-input"),H=document.querySelectorAll(".exercise-button"),l=document.querySelector(".exercise-parts-list"),v=document.querySelector(".exercise-log-span"),p=document.querySelector(".exercise-title-span");document.querySelector(".exercise-part-button");document.querySelector(".pop-add-fav");document.querySelector(".pop-ex-close-btn");document.querySelector(".pop-backdrop");document.querySelector(".backdrop");document.querySelector(".pop-rating-btn");document.querySelector(".pop-up-close-btn");document.querySelector(".pop-ex-info");document.querySelector(".modal123");document.querySelector(".pop-backdrop.is-open");let g,T="",m=[],w,q=0,E=0,I=0,P=0;localStorage.removeItem("searchInput");localStorage.removeItem("searchQuery");sessionStorage.removeItem("data");A.addEventListener("click",async e=>{e.preventDefault(),m=[],g=1;try{const s=(await N(e)).results;m=[...s],p.innerHTML="",p.textContent=`${s[0].target}`.charAt(0).toUpperCase()+`${s[0].target}`.slice(1),sessionStorage.setItem("data",JSON.stringify({results:s})),q=0;for(let c=0;c<8;c++)q++;if(s<8&&(f.innerHTML=""),s.length===0)throw new Error({title:"No Results",message:"No images found. Please try a different search term."});s.length===8&&(f.innerHTML=`   <li class="exercise-page-number">
    ...
    </li>`),p.style.visibility="visible",M.style.display="none",l.style.display="flex",v.style.visibility="visible",l.innerHTML=s.reduce((c,i,a)=>c+`
...
<path fill="#f6f6f6" style="fill: var(--color1, #f6f6f6)" d="M20.919 10.126c1.166 0 2.111-0.924 2.111-2.063s-0.945-2.063-2.111-2.063c-1.166 0-2.111 0.924-2.111 2.063s0.945 2.063 2.111 2.063z"></path>
        ...
        `,"")}catch(t){console.error("Error:",t)}});f.addEventListener("click",async e=>{g=e.target.textContent,w=8;try{const i=`https://energyflow.b.goit.study/api/exercises?muscles=${JSON.parse(localStorage.getItem("searchQuery")).searchQuery}&page=${g}&limit=${w}`,a=await fetch(i);if(a.status!==200)throw new Error("Failed to fetch data. Please try again later.");const o=(await a.json()).results;sessionStorage.setItem("data",JSON.stringify({results:o})),E=0;for(let r=0;r<8;r++)E++;l.innerHTML=o.reduce((r,u,C)=>r+`
        <li class="exercise-parts">
            <div class="part-container">
                <div class="exercise-head-container">
                    <span class="exercise-badge">WORKOUT</span>
...
<path fill="#f6f6f6" style="fill: var(--color1, #f6f6f6)" d="M20.919 10.126c1.166 0 2.111-0.924 2.111-2.063s-0.945-2.063-2.111-2.063c-1.166 0-2.111 0.924-2.111 2.063s0.945 2.063 2.111 2.063z"></path>
        ...
        `,"")}catch(s){console.error("Error:",s)}});H.forEach(e=>{e.addEventListener("click",t=>{t.preventDefault();const s=t.target.textContent;localStorage.removeItem("searchInput"),localStorage.removeItem("searchQuery"),V(t),s==="Muscles"?(b.style.display="none",k.style.display="none",l.style.display="none",M.style.display="flex",v.style.visibility="hidden",p.style.visibility="hidden",f.style.display="flex"):s==="Body parts"?(v.style.visibility="hidden",l.innerHTML="",b.style.display="flex",k.style.display="flex",l.style.display="flex",M.style.display="none",f.style.display="none",p.style.visibility="hidden"):(b.style.display="none",k.style.display="none",l.style.display="none",v.style.visibility="hidden",p.style.visibility="hidden")})});J.addEventListener("submit",async function(e){e.preventDefault(),m=[],g=1;try{const t=b.value,c=(await U(t)).results;if(m=[...c],sessionStorage.setItem("data",JSON.stringify({results:c})),c.length===0){l.innerHTML="",l.insertAdjacentHTML("beforeend","<li class='no-result-list'><p class='no-result'>Unfortunately,<span class='no-result-span'> no results</span> were found.You may want to consider other search options to find the exercise you are looking for. Our range is wide and you have the opportunity to find more options that suit your needs.</p>"),f.style.display="none";return}else{p.innerHTML="",p.textContent=`${c[0].bodyPart}`.charAt(0).toUpperCase()+`${c[0].bodyPart}`.slice(1),p.style.visibility="visible",v.style.visibility="visible",f.style.display="flex",I=0;for(let i=0;i<8;i++)I++;l.innerHTML=c.reduce((i,a,d)=>i+`
        <li class="exercise-parts">
            <div class="part-container">
                <div class="exercise-head-container">
                    <span class="exercise-badge">WORKOUT</span>
...
<path fill="#f6f6f6" style="fill: var(--color1, #f6f6f6)" d="M20.919 10.126c1.166 0 2.111-0.924 2.111-2.063s-0.945-2.063-2.111-2.063c-1.166 0-2.111 0.924-2.111 2.063s0.945 2.063 2.111 2.063z"></path>
        ...
        `,"")}}catch(t){console.error("Error:",t)}});f.addEventListener("click",async e=>{g=e.target.textContent,w=8;try{T=JSON.parse(localStorage.getItem("searchInput")).searchInput;const c=`https://energyflow.b.goit.study/api/exercises?bodypart=${T}&page=${g}&limit=${w}`,i=await fetch(c);if(i.status!==200)throw new Error("Failed to fetch data. Please try again later.");const h=(await i.json()).results;sessionStorage.setItem("data",JSON.stringify({results:h})),P=0;for(let o=0;o<8;o++)P++;l.innerHTML=h.reduce((o,r,u)=>o+`
        <li class="exercise-parts">
            <div class="part-container">
                <div class="exercise-head-container">
                    <span class="exercise-badge">WORKOUT</span>
...
<path fill="#f6f6f6" style="fill: var(--color1, #f6f6f6)" d="M20.919 10.126c1.166 0 2.111-0.924 2.111-2.063s-0.945-2.063-2.111-2.063c-1.166 0-2.111 0.924-2.111 2.063s0.945 2.063 2.111 2.063z"></path>
        ...
        `,"")}catch(s){console.error("Error:",s)}});const y=document.querySelector(".modal123");function z(e){y.innerHTML=`
...
`}l.addEventListener("click",async e=>{e.preventDefault();const t=e.target.id,s=JSON.parse(sessionStorage.getItem("data"));if(e.target.tagName=="BUTTON"&&t==t){const c=s.results[t];z(c),R(c)}});function R(e){function t(h){const o=JSON.parse(localStorage.getItem("exercises"))||[];o.push(e),localStorage.setItem("exercises",JSON.stringify(o)),[...o];const r=document.querySelector(".pop-add-fav");r&&r.removeEventListener("click",t),document.querySelector(".pop-ex-close-btn"),j.success({title:"Excellent choice!",message:"The exercise has been successfully added to your favorites. Get ready for endless inspiration and great results!",position:"center"})}const s=document.querySelector(".pop-add-fav");s?s.addEventListener("click",t):console.error("Button not found");const c=document.querySelector(".pop-backdrop.is-open");async function i(){await z(e),y.innerHTML="",c.classList.remove("is-open")}c&&c.addEventListener("click",i),window.addEventListener("keydown",function(h){h.key==="Escape"&&(y.innerHTML="")});function a(){y.innerHTML=""}const d=document.querySelector(".pop-ex-close-btn");d?d.addEventListener("click",a):console.error("Close button not found")}function V(e){H.forEach(t=>{t.classList.remove("exercise-button-active"),e.target.classList.add("exercise-button-active")})}const n=JSON.parse(localStorage.getItem("exercises")),L=document.querySelector(".fav-list");document.querySelector(".favorites");const x=document.querySelector(".fav-counter"),B=document.querySelector(".empty-content-fav");function S(e){n.length!=0&&B.classList.add("visually-hidden"),L&&(L.innerHTML=n.slice(0,8).map(({bodyPart:t,name:s,target:c,burnedCalories:i,_id:a},d)=>`
    <li class="exercise-parts">
      <div class="part-container">
        <div class="exercise-head-container">
          <span class="exercise-badge">WORKOUT</span>
...
<path fill="#f6f6f6" style="fill: var(--color1, #f6f6f6)" d="M20.919 10.126c1.166 0 2.111-0.924 2.111-2.063s-0.945-2.063-2.111-2.063c-1.166 0-2.111 0.924-2.111 2.063s0.945 2.063 2.111 2.063z"></path>
  ...
  `).join("")),O()}S();function O(){document.querySelectorAll(".exercise-trash-button").forEach(t=>{t.addEventListener("click",s=>{const c=s.currentTarget.getAttribute("data-index");n.splice(c,1),localStorage.setItem("exercises",JSON.stringify(n)),S(),n.length<8&&x.classList.add("visually-hidden"),n.length===0&&B.classList.remove("visually-hidden")})})}S();const D=document.querySelectorAll(".favorite-button");D.forEach(e=>{e.addEventListener("click",t=>{const s=t.currentTarget.id,c=n.find(i=>i._id===s);c&&F(c)})});n.length>8&&W();function W(){x&&(x.innerHTML=`
        <li class="exercise-page-number">
        ...
        </li>`)}x?x.addEventListener("click",async e=>{const t=e.target.id;L.innerHTML=n.slice(t,t+8).map(({bodyPart:s,name:c,target:i,burnedCalories:a,_id:d},h)=>`
    <li class="exercise-parts">
      <div class="part-container">
        <div class="exercise-head-container">
          <span class="exercise-badge">WORKOUT</span>
      ...
      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.6" d="M21.333 8V6.933c0-1.493 0-2.24-.291-2.811a2.666 2.666 0 0 0-1.165-1.165c-.57-.291-1.317-.291-2.811-.291h-2.133c-1.493 0-2.24 0-2.811.291-.502.256-.91.664-1.165 1.165-.291.57-.291 1.317-.291 2.811V8m2.667 7.333V22m5.334-6.667V22M4 8h24m-2.667 0v14.933c0 2.24 0 3.36-.436 4.216a3.9962 3.9962 0 0 1-1.748 1.748c-.856.436-1.976.436-4.216.436h-5.867c-2.24 0-3.36 0-4.216-.436a3.9962 3.9962 0 0 1-1.748-1.748c-.436-.856-.436-1.976-.436-4.216V8"/>
            ...
            <button class="favorite-button" id="${d}">
              Start
              <svg class="exercise-btn-icon" width="14" height="14" viewBox="0 0 32 32">
      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.6" d="m15 28 13-13m0 0L15 2m13 13H2"/>
              </svg>
...
`,document.querySelector(".fav-remove-btn").addEventListener("click",function(c){const i=c.currentTarget.getAttribute("data-index");n.splice(i,1),localStorage.setItem("exercises",JSON.stringify(n)),S(),$.innerHTML="",n.length===0&&B.classList.remove("visually-hidden")}),document.querySelector(".fav-close-btn").addEventListener("click",function(){$.innerHTML=""})}
//# sourceMappingURL=exersises-a299f5e3.js.map
